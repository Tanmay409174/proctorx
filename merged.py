...
phone_model = YOLO("yolov8n.pt")  # or yolov5s.pt if you were using v5
...
def detect_phone(cap):
    """Detect phone using YOLO model"""
    ret, frame = cap.read()
    if not ret:
        return False
    ...
    for box, cls in zip(results[0].boxes.xyxy, results[0].boxes.cls):
        label = phone_model.names[int(cls)].lower()

        if "cell phone" in label:
            print("[monitor] 📱 Phone detected! Terminating exam...")
            return True
    return False
...
def main():
    ...
    try:
        while True:
            ...
            if detect_phone(cap):
                call_terminate_endpoint()  # Terminate the exam if phone is detected
